...
class Broadcaster:
    ...
    async def run_account(self, bot: Client, phone: str, status_msg: Message):
        ...
        try:
            ...
            self._running[phone] = False
            # Refresh the account panel after stopping
            try:
                from bot import show_account
                await show_account(status_msg, phone)
            except Exception:
                await self._edit(status_msg,
                    f"🛑 <b>{self.db.get_account(phone or {}).get('label', phone)}</b> stopped.")
